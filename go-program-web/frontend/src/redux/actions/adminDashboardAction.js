import { ADMIN_DASHBOARD_GET_PENDING_APPROVAL_EVENTS_SUCCESS, ADMIN_DASHBOARD_GET_PENDING_APPROVAL_EVENTS_FAILED, 
    ...
    ADMIN_DASHBOARD_GET_PENDING_APPROVAL_SUGGESTED_EVENTS_FAILED } from './types';
...
export const getPendingApprovalSuggestedEvents = () => dispatch => {
    const token = localStorage.getItem('token');
    fetch(`${backendUrl}/admin/dashboardPendingApprovalSuggestedEvents`,{
        headers: {
            'Content-Type': 'application/json',
            Accept: 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    .then(res => {
        if(res.status === 200){
            res.json().then(data => {
                dispatch({
                    type: ADMIN_DASHBOARD_GET_PENDING_APPROVAL_SUGGESTED_EVENTS_SUCCESS,
                    payload: data
                });
            });
        }else{
